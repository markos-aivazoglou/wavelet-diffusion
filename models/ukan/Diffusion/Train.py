...
def train(modelConfig: Dict):
    ...
    if modelConfig["dataset"] == "cvc":
        dataset = UnlabeledDataset(
            "data/cvc/images_64/",
            transform=transform,
            repeat_n=modelConfig["dataset_repeat"],
        )
    ...
    # model setup
    net_model = model_dict[modelConfig["model"]](
        T=modelConfig["T"],
...
def eval_tmp(modelConfig: Dict, nme: int):
    ...
    with torch.no_grad():
        device = torch.device(modelConfig["device"])
        model = model_dict[modelConfig["model"]](
            T=modelConfig["T"],
            ch=modelConfig["channel"],
            ch_mult=modelConfig["channel_mult"],
            attn=modelConfig["attn"],
        ...
        save_image(
            ...
            os.path.join(
                save_root,
                modelConfig["sampledImgName"].replace(".png", "_{}.png").format(nme),
            ),
        ...
        for i, image in enumerate(sampledImgs):
            ...
            save_image(
                ...
                os.path.join(
                    ...
                    modelConfig["sampledImgName"].replace(".png", "_{}.png").format(i),
                ),
                nrow=modelConfig["nrow"],
            )
